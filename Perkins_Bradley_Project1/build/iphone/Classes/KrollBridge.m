...
NSString * Perkins_Bradley_Project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Perkins_Bradley_Project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_perkins_bradley_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Perkins_Bradley_Project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Perkins_Bradley_Project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_perkins_bradley_project1 = [[Perkins_Bradley_Project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_perkins_bradley_project1];
	
	NSString *_perkins_bradley_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _perkins_bradley_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_perkins_bradley_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Perkins_Bradley_Project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.perkins_bradley_project1.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.perkins_bradley_project1.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
