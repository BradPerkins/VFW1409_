...
NSString * Perkins_Bradley_Project4$ModuleRequireFormat = @"(function(exports){"
...
@implementation Perkins_Bradley_Project4Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_perkins_bradley_project4);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Perkins_Bradley_Project4 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Perkins_Bradley_Project4" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_perkins_bradley_project4 = [[Perkins_Bradley_Project4Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_perkins_bradley_project4];
	
	NSString *_perkins_bradley_project4NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _perkins_bradley_project4NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_perkins_bradley_project4 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Perkins_Bradley_Project4$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.perkins_bradley_project4.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.perkins_bradley_project4.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
